...
def upload_task(user_obj, file_data, file_name, notes_obj, blob_size, audio_request_id):
    ...
    if data:
        ...
        if user_obj.plan in direct_api_supported_plans:
            ...
            stt_data = api_call(file_to_process=file_data, end_point=stt_end_point, f_align_end_point=f_align_end_point,
                                sound_recog_endpoint=sound_recog_endpoint, binary=True, file_name=file_name)
            print("stt_data")
            print(stt_data)
            if stt_data:
                audio_model_obj = audio_save_to_db(file_size=blob_size, stt_data=stt_data, note_obj=notes_obj, url=url)
                """websocket code here"""
                to_send_ws_data = {
                    "status": "PROCESSED",
